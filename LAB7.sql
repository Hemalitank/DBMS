CREATE TABLE EMP(
EID INT,ENAME  VARCHAR(50),DEPARTMENT VARCHAR(50),SALARY INT, JOININGDATE DATETIME,CITY VARCHAR(50)
)
...
SELECT COUNT(EID) AS EMPLOYEE_TOL FROM EMP
...
SELECT COUNT(JOININGDATE) AS COUNT_DATE FROM EMP WHERE JOININGDATE>'1991-02-08' 
--7. Display average salary of Admin department.
SELECT AVG(SALARY)AS ADMIN_SALARY FROM EMP WHERE DEPARTMENT='ADMIN' GROUP BY DEPARTMENT
--8. Display total salary of HR department.
SELECT SUM(SALARY) AS TOTAL_SAL FROM EMP WHERE DEPARTMENT='HR' GROUP BY DEPARTMENT 
...
SELECT MAX(SALARY) AS MAX_SAL FROM EMP WHERE CITY='AHMEDABAD' GROUP BY CITY
--12. Find city wise highest salary.
SELECT MAX(SALARY) AS MAX_SAL FROM EMP GROUP BY CITY
--13. Find department wise lowest salary.
SELECT MIN(SALARY) AS MIN_SAL FROM EMP GROUP BY DEPARTMENT
...
SELECT COUNT(EID)AS COUNT_CITY FROM EMP WHERE CITY='RAJKOT' GROUP BY CITY
--2. Display the difference between the highest and lowest salaries. Label the column DIFFERENCE.
SELECT MAX(SALARY)-MIN(SALARY)AS DEFFRENCE_SALARY FROM EMP 
--3. Display the total number of employees hired before 1st January, 1991.
SELECT COUNT(EID) AS COUNT_NUM FROM EMP WHERE JOININGDATE<'1991-01-01'
--Part – C:
--1. Count the number of employees living in Rajkot or Baroda.
SELECT COUNT(EID) AS COUNT_CITY FROM EMP WHERE CITY IN('RAJKOT','BARODA')
--2. Display the total number of employees hired before 1st January, 1991 in IT department.
SELECT COUNT(EID) AS COUNT_EMP FROM EMP WHERE JOININGDATE<'1991-01-01' AND DEPARTMENT='IT'
...
SELECT ENAME,MAX(SALARY) AS MAX_SAL FROM EMP GROUP BY DEPARTMENT
